...
final class CLIPTokenizerTests: XCTestCase {
    ...
    /// split by merge rank. The split is deliberately non-obvious — "only|dav|es", not the
    /// "only|da|ves" a reader might assume — which is exactly why it is pinned to the reference.
    func testWordAbsentFromVocabularySplitsByMergeRank() {
        assertTokens("onlydaves", [49406, 11646, 3700, 542, 49407])
    }
